...
# Clean up temporary files and script on exit
...
echo -e "${cyan}===========================================${rest}"
echo -e "${yellow}Example VLESS config URL:${rest}"
echo -e "${yellow}vless://abcd1234-56ef-7890-ghij-1234567890@example.com:443?security=tls&host=example.com#MyVless${rest}"
echo ""
...
process_vless_config() {
    local vless_url="$1"
    local endip_v4="$2"
    # Extract host from between '@' and ':' or from 'host=' parameter
    local host=$(echo "$vless_url" | grep -oP '(?<=@)[^:]+' || echo "$vless_url" | grep -oP '(?<=host=)[^&]*')
    # Extract port from between ':' and '?'
    local port=$(echo "$vless_url" | grep -oP '(?<=:)[0-9]+(?=\?)')
    ...
    local new_vless_url=$(echo "$vless_url" | sed -E "s/@[^:]+:[0-9]+\?/@$address:$port?/")
